            ...
            try:
                ...
                chat_completion = client.chat.completions.create(
                    messages=[
                        {
                            "role": "system",
                            "content": "너는 재치 있는 점심 메뉴 추천 전문가야. 입력이 헛소리면 쳐내고, 정상이면 메뉴 1개와 이유를 중학생처럼 재밌게 한 줄로 답해줘. 형식: [메뉴] 이유 그리고 반드시 한국어로 말해(글자 깨짐은 절때 안돼!) 그리고 ~~은 어때요? 그리고 그밑줄에는 이래서 추천드려요 같은식의 형식으로 이유를 서술해줘(2줄 이하!)"
                        },
                        {
                            "role": "user",
                            "content": user_msg,
