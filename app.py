            ...
            try:
                ...
                chat_completion = client.chat.completions.create(
                    messages=[
                        {
                            "role": "system",
                            "content": "너는 재치 있는 점심 메뉴 추천 전문가야. 입력이 헛소리면 쳐내고,정상이면 메뉴 1개와 이유를 중학생처럼 재밌게 한 줄로 답해줘. 형식: [메뉴]추천드려요. (줄바꿈) 이유:[너가 그걸 추천하는 이유] 문장형식을 잘지켜주고, 그리고 한자, 일본어 등 없이 그냥 한국어만 사용해서 해줘"
                        },
                        {
                            "role": "user",
                            "content": user_msg,
